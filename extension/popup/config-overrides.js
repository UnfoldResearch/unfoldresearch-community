...
var overridePath = function (config) {
  var oneOfRule = config.module.rules.find(function (rule) {
    return rule.oneOf;
  });
  if (!oneOfRule) {
    return config;
  }
  var tsRule = oneOfRule.oneOf.find(function (rule) {
    return rule.test && rule.test.toString().includes('ts');
  });
  if (!tsRule) {
    return config;
  }
  var newIncludePaths = [path.resolve(__dirname, '../../packages/unfold-core'), path.resolve(__dirname, '../../../packages/unfold-ui')];
